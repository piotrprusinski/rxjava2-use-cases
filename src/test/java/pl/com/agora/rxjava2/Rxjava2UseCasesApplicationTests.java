...
public class Rxjava2UseCasesApplicationTests {
	...
	public void shouldCreateSimpleObservableFromList() {
		Observable.just(Lists.newArrayList("t1", "t2", "t3")).subscribe(this::log);
	}
	...
	public void shouldRunTaskInThread() {
		Observable.fromIterable(Lists.newArrayList("t1", "t2", "t3")).subscribeOn(Schedulers.single())
				.subscribe(this::log);
		// sleep(1);
	}
	...
	public void shouldMakeKebab() throws Exception {
		...
		Observable.zip(meatObservable, breadObservable, vegetablesObservable,
	...
	public void shouldMakeKebabNotWorkingAsync() throws Exception {
		...
		Observable<String> meatObservable = Observable.range(1, 3)
				.flatMap(i -> Observable.fromCallable(() -> meatRepo.makeMeat(i))).subscribeOn(Schedulers.io());
